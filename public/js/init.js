  ...
  var gif = {
    init: function () {
      $(".gif").each(function () {
        ...
        // 在图片加载过程中保证DOM不会被重排
        var $stop = $(
          '<span class="stop"><span class="iconfont icon-stop"></span></span>'
        ).hide();
        var $play = $(
          '<span class="play"><span class="iconfont icon-play"></span></span>'
        ).hide();
        var $loading = $(
        ...
        function calcGifSize() {
          ...
          if (width && height) {
            ...
            if ($gif.data("init") === true) {
              ...
              $canvas[0]
                .getContext("2d")
                .drawImage($gif[0], 0, 0, width, height);
            }
          }
        }
    ...
    init: function () {
      ...
      $allImg.each(function () {
        ...
        if ($(this).attr("data-desc")) {
          var $imgDesc = $('<span class="desc">').text(
            $(this).attr("data-desc")
          );
      ...
      function calcSize() {
        ...
        $imgPreviewContainer.css(
          "width",
          EACH_PREVIEW_WIDTH * $imgPreviewList.length
        );
      ...
      function prePic() {
        if (!imgList.isDuringScroll) {
          ...
          imgList.activeIndex--;
          var i = (imgList.activeIndex =
            imgList.activeIndex < 0
              ? imgList.allLength - 1
              : imgList.activeIndex);
          imgJumpTo(i);
          progressJumpTo(i);
      ...
      function nextPic() {
        if (!imgList.isDuringScroll) {
          ...
          imgList.activeIndex++;
          var i = (imgList.activeIndex =
            imgList.activeIndex > imgList.allLength - 1
              ? 0
              : imgList.activeIndex);
          imgJumpTo(i);
          progressJumpTo(i);
    ...
    init: function () {
        ...
        function ($activeLink) {
          ...
          if (linkTop > containerHeight || linkTop < 0) {
            $($scrollContainer).animate(
              {
                scrollTop:
                  $activeLink[0].offsetTop - containerHeight / 2 + "px",
              },
              {
                duration: 100,
                easing: "swing",
      ...
      function syncLink(
        ...
        function handleLinkScroll() {
          ...
          currTop =
            typeof currTop !== "undefined" ? currTop : window.pageYOffset;
          desPosArray.some(function (key, index) {
            if (currTop + 1 < key) {
              ...
              if (activeIndex !== currActiveIndex && activeIndex >= 0) {
                ...
                $.isFunction(activeCallback) &&
                  activeCallback($activeLink, currActiveIndex);
              }
              return true;
            }
            return false;
  ...
  var safe = {
    ...
    inSafeFrame: function () {
      if (window.top !== window && document.referrer) {
        ...
        if (!endsWith(host, ".ede.ink") && !endsWith(host, "localhost")) {
          top.location.href = "https://ede.ink";
        }
      }
    },
  };
  ...
  var imgScale = {
    init: function () {
      ...
      $(".article img").dblclick(function () {
        var $modal = $(".img-modal");
        var $body = $("body");
        $modal = $modal.length > 0 ? $modal : $("<div>").addClass("img-modal");
        $modal.empty();
        var rect = $(this)[0].getBoundingClientRect();
        var $copyImg = $('<img class="img-copy">').attr(
          "src",
          $(this).attr("src")
        );
        // 设置原始位置
        ...
        function scale() {
          var padding = 80;
          var widthZoom = (window.innerWidth - padding * 2) / rect.width;
          var heightZoom = (window.innerHeight - padding * 2) / rect.height;
          var minZoom = Math.min(widthZoom, heightZoom);
          $copyImg.css({
            top: padding + 30,
            left: (window.innerWidth - rect.width * minZoom) / 2,
            // right: padding,
            // bottom: padding,
            width: rect.width * minZoom,
            height: rect.height * minZoom,
            // transform: 'scale(' + Math.round(minZoom * 100) + '%)',
          });
        }
        // setTimeout: 让动画效果生效
        setTimeout(function () {
        ...
        function hide() {
          ...
          $(window).off("keydown", hideOnKeydown);
          $modal.off("click", hide);
        }
        function hideOnKeydown(e) {
        ...
        $(window).on("keydown", hideOnKeydown);
        $modal.on("click", hide);
      });
    },
  };
